...
class SellSubscriptionTests(TestCase):
    def setUp(self):
        ...
        self.admin = User.objects.create_user(
            phone="77001112233", password="pass", full_name="Админ",
            organization=self.org, role=User.Role.ADMIN,
        )
        self.st = create_type(self.org, name="8 занятий", price=25000,
                               quota_sessions=8, duration_days=30, directions=[self.ballet])

    def _sell(self, **overrides):
        kwargs = dict(
            actor=self.admin, child=self.child, subscription_type_version=self.st.versions.latest(),
            direction=self.ballet, starts_on=date.today(), ends_on=date.today() + timedelta(days=30),
            paid_amount=25000, payment_method=Payment.Method.KASPI_TRANSFER,
        )
    ...
    def test_discount_with_reason_applied(self):
        sub, _ = self._sell(
            discount_amount=5000, discount_reason=Subscription.DiscountReason.SECOND_CHILD,
            paid_amount=20000,
        )
