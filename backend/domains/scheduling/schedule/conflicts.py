...
не запрет. Отменённые занятия конфликтов не создают. Индивидуальные
занятия (group=None) участвуют наравне с групповыми — конфликт определяется
по room/teacher, а не по группе.
"""
...
def compute_conflict_map(lessons):
    ...
    у которых есть хотя бы один конфликт. Отменённые занятия исключаются
    и не могут ни с кем конфликтовать."""
